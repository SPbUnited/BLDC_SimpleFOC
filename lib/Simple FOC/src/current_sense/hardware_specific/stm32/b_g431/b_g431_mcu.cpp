...
volatile uint16_t adcBuffer1[ADC_BUF_LEN_1] = {
    0};  // Buffer for store the results of the ADC conversion
volatile uint16_t adcBuffer2[ADC_BUF_LEN_2] = {
    0};  // Buffer for store the results of the ADC conversion
...
void _configureOPAMPs(OPAMP_HandleTypeDef* OPAMPA, OPAMP_HandleTypeDef* OPAMPB,
                      OPAMP_HandleTypeDef* OPAMPC)
{
...
void MX_DMA1_Init(ADC_HandleTypeDef* hadc, DMA_HandleTypeDef* hdma_adc,
...
void* _configureADCInline(const void* driver_params, const int pinA, const int pinB,
                          const int pinC)
{
    ...
    SIMPLEFOC_DEBUG(
        "B-G431B does not implement inline current sense. Use low-side current sense instead.");
    return SIMPLEFOC_CURRENT_SENSE_INIT_FAILED;
}

void* _configureADCLowSide(const void* driver_params, const int pinA, const int pinB,
                           const int pinC)
{
    _UNUSED(driver_params);
...
void* _driverSyncLowSide(void* _driver_params, void* _cs_params)
{
    ...
    bool tim_interrupt =
        _initTimerInterruptDownsampling(cs_params, driver_params, adc_interrupt_config[adc_index]);
    if (tim_interrupt)
    {
        // error in the timer interrupt initialization
        SIMPLEFOC_DEBUG(
            "STM32-CS: timer has no repetition counter, ADC interrupt not supported for B-G431");
        return SIMPLEFOC_CURRENT_SENSE_INIT_FAILED;
    }
