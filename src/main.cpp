...
LowsideCurrentSense current_sense = LowsideCurrentSense(0.003f, -20.0f, A_OP1_OUT, A_OP2_OUT, A_OP3_OUT);
...
motor.PID_velocity.D = 0;//0.0035; //0.0035
...
void loop() {
  ...
  // }
   ...
   if(use == false)
   {
    // if(millis() - last_receive_timer < 10000)
    // {
      motor.current_limit = max_current;
      motor.PID_velocity.P = 0.05;
